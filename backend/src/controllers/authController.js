...
//             if (user.role !== 'guest') return res.status(403).json({ message: "This account is not a guest account." });
//             if (user.name.toLowerCase() !== name.toLowerCase().trim()) {
//                 return res.status(401).json({ message: "Invalid assigned name for this guest account!" });
//             }
//         } else if (loginType === 'owner') {
//             if (user.role === 'guest') {
//                 return res.status(403).json({ message: "Guest accounts must use the Guest Access login tab." });
//             }
//         }
//         //Password match check karna 
//         const isMatch = await user.matchPassword(password);
